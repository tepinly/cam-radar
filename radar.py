...
while True:
    ...
    #Object Tracking
    boxes_ids = tracker.update(detections, frameCount)
    for box_id in boxes_ids:
        x,y,w,h,id = box_id


        if(tracker.getsp(id)<tracker.limit()):
            cv2.putText(roi,"Id:"+str(id)+" | "+str(tracker.getsp(id))+"km/h",(x,y-15), cv2.FONT_HERSHEY_PLAIN,1,(255,255,0),2)
            cv2.rectangle(roi, (x, y), (x + w, y + h), (0, 255, 0), 3)
        else:
            cv2.putText(roi,"Id:"+str(id)+ " "+str(tracker.getsp(id))+"km/h",(x, y-15),cv2.FONT_HERSHEY_PLAIN, 1,(0, 0, 255),2)
